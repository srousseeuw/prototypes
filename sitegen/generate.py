...
def render_bakery(brief: dict) -> str:
...
def build_teams(items):
    lis = [f'<li>{esc(i)}</li>' for i in items]
    return "\n".join(lis)

def render_sportclub(brief: dict) -> str:
    name = esc(brief["business_name"])
    category = esc(brief.get("category", ""))
    tagline = esc(brief.get("tagline", ""))
    address = esc(brief.get("address", ""))
    ...
    agenda_html = build_agenda_rows(brief.get("agenda", []))
    teams_html = build_teams(brief.get("teams", []))
    highlights_html = build_highlights(brief.get("highlights", []))
    maps_query = address.replace(" ", "+")
...
def main():
    ...
    render_fn = RENDERERS.get(sector)
    if render_fn is None:
        print(f"Onbekende sector '{sector}'. Beschikbaar: {', '.join(RENDERERS)}")
        sys.exit(1)
