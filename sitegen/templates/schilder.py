...
# Een klassieke "verfkaart"-waaier: elke dienst krijgt een eigen kleurstaal,
# zodat het aanbod oogt als de kleurenkaart die een schilder zelf meebrengt.
_SWATCHES = ["#2954c8", "#e2a63b", "#c8483f", "#3f8f5f", "#7a5ac2", "#1b1b1f"]


def _build_swatch_services(items):
    cards = []
    for i, item in enumerate(items):
        color = _SWATCHES[i % len(_SWATCHES)]
        cards.append(
            f'<li class="swatch-card"><span class="swatch-chip" style="background:{color}"></span>'
            f'<span class="swatch-label">{esc(item)}</span></li>'
        )
    return "\n".join(cards)
...
def render(brief: dict) -> str:
    ...
    services_html = _build_swatch_services(brief.get("services", []))
    maps_q = _maps_query(address)

    eyebrow = esc(brief.get("eyebrow", f"Schilder- en glaszetbedrijf · sinds {since}" if since else "Schilder- en glaszetbedrijf"))
    ...
    hours_html = f'<p class="hours-note-text">{hours_note}</p>' if hours_note else ""
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* Signature element: een waaier van verfstalen, als een echte kleurenkaart. */
  .fan-strip {{
    display: flex;
    height: 14px;
  }}
  .fan-strip span {{ flex: 1; }}
  ...
  .section-sub {{ color: var(--ink-soft); margin-bottom: 28px; max-width: 56ch; }}
  ...
  .about-card p {{ margin-bottom: 14px; color: #333333; }}
  ...
  .info-row {{ display: flex; justify-content: space-between; gap: 12px; padding: 6px 0; font-size: 0.92rem; border-bottom: 1px solid rgba(255,255,255,0.14); }}
  .info-row span:first-child {{ color: #c8cbe0; }}
  .info-row a {{ text-decoration: none; color: #fff; font-weight: 600; }}

  .hours-block {{ margin-top: 18px; }}
  .hours-note-text {{ font-size: 0.88rem; color: #d8dcf0; }}
...
<div class="fan-strip">
  <span style="background:#2954c8"></span><span style="background:#e2a63b"></span>
  <span style="background:#c8483f"></span><span style="background:#3f8f5f"></span>
  <span style="background:#7a5ac2"></span><span style="background:#1b1b1f"></span>
</div>

<header class="top">
  <div class="wrap">
    <div class="brand">{name.split()[0] if name else ""}<span> {" ".join(name.split()[1:])}</span></div>
...
<section id="diensten">
  <div class="wrap">
    <h2>Onze diensten</h2>
    <p class="section-sub">Eén vakman voor de hele klus — van eerste kwast tot laatste likje verf.</p>
...
<section id="over" style="background:#fff; border-top:1px solid var(--line); border-bottom:1px solid var(--line);">
...
<section class="cta-band">
  <div class="wrap">
    <h2>Toe aan een frisse muur of nieuw kozijn?</h2>
    <p style="color:#dbe1f7;">{"Bel of mail" if email else "Bel"} voor een vrijblijvende offerte op maat.</p>
    <a class="btn btn-primary" href="tel:{phone}">{cta}</a>
  </div>
</section>
